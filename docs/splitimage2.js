var select = {
	file: getElId("fileSel"),
	scale: getElId("scaleSel"),
	left: getElId("leftSel"),
	right: getElId("rightSel"),
	...
	function scaleCanvas(inCanvas, side) {
		...
		resizer
			...
			.then(function () {
				...
				updateslider()
				window.location.hash = viewOptions.file.concat(viewOptions.scale, "&", viewOptions.left, "=", viewOptions.leftQ, "&", viewOptions.right, "=", viewOptions.rightQ, "&", viewOptions.subset)
			})
			.catch(function (err) {
				console.log(err)
				throw err
...
function setSide(side) {
	...
	setImage(side.toLowerCase(), pathBase, image, function (kbytes, bpp) {
...
function switchMode(keyCode) {
	...
	if (keyCode && keyCode == "16") {
		...
		infoText.center.innerHTML = getSelValue(select[switchTo], "folder") + " " + infoText[switchTo].innerHTML.replace(/&nbsp;/g, "").replace(/←|→/g, "")
